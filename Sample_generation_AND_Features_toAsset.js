...
var testingS_crop = ee.FeatureCollection("users/JY54/230801/YRDcrop2023"),
    ...
    Sample = ee.FeatureCollection("users/JY54/crop240429/SourceSample0805_2");
...
// var TargetROI = YRD;
var id = 3
var TargetROI = Blocks.filter(ee.Filter.eq('ID',id)).geometry();
Map.addLayer(TargetROI,{},'TargetROI',0)
var Th_sos = 0.3;
var Th_eos = 0.3;
var Th_sdps = 0.7;
var Th_edps = 0.4; 
...
{  
  ...
  var OtherSeason_Col = NDVI_10dCol.map(function(im) {
    ...
    var maskDOYGT183 = im.select('DOY').lte(SOS); // 如果DOY大于183
    var maskDOYLTE183 = im.select('DOY').gte(EOS); // 如果DOY小于等于183
    var mask = condition.where(condition, maskDOYGT183).where(condition.not(), maskDOYLTE183);
  ...
  var maxND = col_MainSeason.qualityMosaic('NDVI').select('DOY');
  var VV_GSmean = filterByRange(S1,SDPS,EDPS,'DOY').select('VV').mean().rename('VV_GSmean').unmask(-10)
  ...
  var NDVI_Integral = filterByRange(composites_cloud,SOS,maxND,'DOY').select('NDVI')
  NDVI_Integral = NDVI_Integral.sum().multiply(maxND.subtract(SOS)).rename('NDVI_Integral')
}

// 耕地淹没 
  var NDVImax = composites_cloud
                    //.map(function(image){return image.updateMask(image.select('NDVI').lt(0.75));})
                    .select('NDVI').max().rename('NDVImax').clip(TargetROI);
  var vegetation1 = ee.Image(1).updateMask(NDVImax.gt(0.35));
  
  // Map.addLayer(vegetation1.clip(TargetROI),{min:1, max:1,palette:'#1cc219'}, 'vegetation1',true);
  //forest detection
...
var features = VV_GSmean.addBands(NDVI_Integral).addBands(LSWI_Transmean.sum()).addBands(SecPeak).addBands(GUS).addBands(SOS).addBands(EOS).addBands(GSL).addBands(SDPS)
                .clip(TargetROI).updateMask(cropmask);

  
  var CropProperties = ["GUS","SecPeak","TransF","VV_GSmean",'NDVI_Integral']
  ...
  var rf_classifier= ee.Classifier.smileRandomForest(80).train(Sample, 'class',CropProperties,0.5,20);
...
Map.addLayer(classified1.clip(TargetROI),{max:6,min:1,palette:palette},'classified1',0)
  ...
  var cla_confidentIC = cla_probIC.max().updateMask(cla_probIC.max().gt(0.70)).clip(TargetROI)
  var cla_sampleimg = classified1.updateMask(cla_confidentIC).unmask(0)
  

{
  // 使用focal_mode方法来计算每个像素的8邻域众数
  var mode = cla_sampleimg.focalMode({
    radius: 3,  // 半径1表示考虑8邻域
    units: 'pixels',
    kernelType: 'square',  // 使用方形卷积核
    iterations: 1
  });
  
  // 创建布尔掩膜，保留像素值与其众数相同的点
  var mask = cla_sampleimg.eq(mode);
  
  // 应用掩膜
  var filteredImage = cla_sampleimg.updateMask(mask).selfMask();
  
  
}

Map.addLayer(filteredImage.clip(TargetROI),{max:6,min:1,palette:palette},'filteredImage')
cla_sampleimg = filteredImage.clip(TargetROI)
  ...
  var newSample = features.sampleRegions({collection: samples, properties: ['classification'], scale: 30,tileScale:8,geometries:true});

  var RegionProperties = ["VV_GSmean","TransF","SecPeak","GUS","SoS","EoS","GSL","SDPS"];
//Map.addLayer(samples,{},'Samples',0)  
Export.table.toAsset({
  collection:newSample,
  description:'AA_NCPSample'+id,
  assetId:'users/JY54/NCPSamples/NCPSample'+id
});
Export.table.toDrive({
  collection:newSample,
  description:'DD_NCPSample'+id,
  folder:'NCPSamples',
  fileNamePrefix:'NCPSample'+id});
Export.image.toAsset({
  image:features,
  description:'AA_NCPFeature'+id,
  assetId:'users/JY54/NCPFeatures/NCPFeature'+id,
  scale:10,
  region:TargetROI,
  maxPixels:1e11
});

Map.addLayer(features,{},'features',0);

var Samples_chart = ee.FeatureCollection([
                      ee.FeatureCollection(ee.Feature(newSample.filter(ee.Filter.eq('classification',1)).geometry(),{'type':1,'class':'corn'})),
                      ee.FeatureCollection(ee.Feature(newSample.filter(ee.Filter.eq('classification',2)).geometry(),{'type':2,'class':'cotton'})),
                      ee.FeatureCollection(ee.Feature(newSample.filter(ee.Filter.eq('classification',3)).geometry(),{'type':3,'class':'rice'})),
                      ee.FeatureCollection(ee.Feature(newSample.filter(ee.Filter.eq('classification',4)).geometry(),{'type':4,'class':'wheat_corn'})),
                      ee.FeatureCollection(ee.Feature(newSample.filter(ee.Filter.eq('classification',5)).geometry(),{'type':5,'class':'wheat_bean'})),
                      ee.FeatureCollection(ee.Feature(newSample.filter(ee.Filter.eq('classification',6)).geometry(),{'type':6,'class':'wheat_rice'}))
                    ]).flatten()
                    
var chart =
    ui.Chart.image
        .doySeriesByRegion({
          imageCollection:composites_cloud.select(['NDVI']),
          bandName:"NDVI",
          regions: Samples_chart,//.filter(ee.Filter.eq('Type',1)),
...
print(newSample);
print(newSample.filter(ee.Filter.eq('classification',1)),'corn');
print(newSample.filter(ee.Filter.eq('classification',2)),'cotton');
print(newSample.filter(ee.Filter.eq('classification',3)),'rice');
print(newSample.filter(ee.Filter.eq('classification',4)),'wheat-corn');
print(newSample.filter(ee.Filter.eq('classification',5)),'wheat-bean');
print(newSample.filter(ee.Filter.eq('classification',6)),'wheat-rice');
